...
func before_each():
    ...
    planner.declare_actions([Callable(planner.current_domain, "create_room"), Callable(planner.current_domain, "place_furniture"), Callable(planner.current_domain, "change_layout")])
    planner.declare_task_methods("design_room", [Callable(planner.current_domain, "create_room"), Callable(planner.current_domain, "place_furniture")])
    planner.declare_task_methods("redesign_apartment", [Callable(planner.current_domain, "change_layout")])

# func test_college_town_plan():
#     planner.verbose = 0
#     var town_state: Dictionary = {} # TODO
#     var town_state = town_state.duplicate(true)
#     var result = planner.find_plan(town_state.duplicate(true), [
