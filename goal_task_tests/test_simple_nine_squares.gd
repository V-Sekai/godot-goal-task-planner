...
func find_path(state, p, current_location, destination, path):
	...
	for next_location in types["location"]:
		...
		if goal_time < INF and not path_has_location(path, next_location):
			...
			var original_time = state.time[p]
			var original_loc = state.loc[p]

			state.time[p] = goal_time
			state.loc[p] = next_location

			var result = find_path(state, p, next_location, destination, new_path)
			if result.size() > 0:
				return result

			# Backtrack: Restore the original state if the path was not found
			state.time[p] = original_time
			state.loc[p] = original_loc
	return []
