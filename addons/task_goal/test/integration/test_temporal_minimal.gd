...
func _validate_task_constraints(mia: PlanResource, task_name: String, start_time: int, duration: int) -> void:
	...
	assert_true(mia.simple_temporal_network.constraints.size() > 0, "Constraints: %d" % mia.simple_temporal_network.constraints.size())
...
func _create_resource_with_initial_constraint() -> PlanResource:
	...
	for c in mia.simple_temporal_network.constraints:
...
func test_propagate_constraints() -> void:
	...
	print("STN before adding initial constraint: %s" % str(mia.simple_temporal_network.to_dictionary()))
	mia.simple_temporal_network.add_temporal_constraint(task_constraints)
	print("STN after adding initial constraint: %s" % str(mia.simple_temporal_network.to_dictionary()))
